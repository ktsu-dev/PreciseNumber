...
internal static partial class BenchmarkHistory
{
	...
	[
		...
		("ConversionBenchmarks.ToDouble", null, "ToDouble"),
	];
	...
	[
		("AbstractionCostBenchmarks.PreciseAdd", "AbstractionCostBenchmarks.BareAdd", "Add"),
		("AbstractionCostBenchmarks.PreciseMultiply", "AbstractionCostBenchmarks.BareMultiply", "Multiply"),
	];
	...
	private static int Ingest(Dictionary<string, string> options)
	{
		...
		if (benchmarks.Count == 0)
		{
			...
			// threw. An older package whose API these benchmarks compile against but whose
			// behaviour they cannot exercise does exactly this. Recording it would put a release
			// on the axis with nothing under it, which reads as a release that was measured and
			// found to cost nothing.
			Console.Error.WriteLine(
	...
	{
		svg.AppendLine(CultureInfo.InvariantCulture, $"""<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}" role="img" aria-label="PreciseNumber allocation and relative time per release">""");
		...
		svg.AppendLine(CultureInfo.InvariantCulture, $"""<text x="{Left}" y="28" class="title">PreciseNumber performance by release</text>""");
	...
	private static void Footer(StringBuilder svg, JsonArray entries, string[] labels, int axisY)
	{
		...
		svg.AppendLine(CultureInfo.InvariantCulture, $"""<text x="{Left}" y="{axisY + 16}" class="caption">Measured on {Escape(measured)}. Full tables: PreciseNumber.Benchmarks.</text>""");
	}
